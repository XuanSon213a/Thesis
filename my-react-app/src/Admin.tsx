...
const CartShopping =<FontAwesomeIcon icon={faCartShopping} />
...
const handleDelete = () => {
  axios.get('http://localhost:3300/logout', { withCredentials: true }) // Use the correct API route
    .then((res) => {
      if (res.data.Status === 'Success') {
        ...
        navigate('/page'); // Redirect to the login page
      }
    })
    .catch((err) => console.log(err));
};
      ...
      <Link to={'/'} className="text-4xl text-white font-mono font-bold hover:underline"  >Alumni's IU Network</Link>
        <div className="header-right">
          ...
          <div>
            ...
            <button className="align-middle select-none text-center ml-10  py-3 px-6 rounded-lg bg-gray-900 text-white shadow-md shadow-gray-900/10 " onClick={handleDelete} >Logout</button>
          </div>
        :
          <div>
              <h3>{message}</h3>
 ...
 {/* content */}
    <section className="flex flex-row ">
        ...
        <div className="basis-1/2 pt-14 text-3xl font-serif bg-[#EDE8E0] text-center ">International University Alumni Network helps you introduce your professional expertise, search for potential business partners, and expand your personal network by chatting with other members without sending a friend request.</div>
