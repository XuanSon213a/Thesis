...
const NewsAll = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await axios.get('http://localhost:3300/api/news/all');
        const newsWithTagsArray = response.data.map((news: any) => ({
          ...news,
          tags: typeof news.tags === 'string' ? news.tags.split(',') : news.tags,
        }));
        setNews(newsWithTagsArray);
      } catch (error) {
  ...
  const uniqueTags = Array.from(new Set(news.flatMap(news => news.tags)));
  const uniqueLocations = Array.from(new Set(news.map(news => news.location)));
  const filteredNews = news.filter(news => {
    const matchesSearchTerm = news.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    news.tags.some(tag => (tagDescriptions[tag] || tag).toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesTag = selectedTag === "" || news.tags.includes(selectedTag);
    const matchesLocation = selectedLocation === "" || news.location === selectedLocation;
  
    // Ensure the time comparison uses the same format
    const newsDate = new Date(news.time).toISOString().split('T')[0]; // Convert event.time to YYYY-MM-DD
    const matchesTime = selectedTime === "" || newsDate === selectedTime;
