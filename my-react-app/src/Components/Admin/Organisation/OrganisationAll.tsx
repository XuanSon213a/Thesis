 ...
 const [searchTerm, setSearchTerm] = useState<string>("");
  ...
  useEffect(() => {
    const fetchOrg = async () => {
      try {
        const response = await axios.get('http://localhost:3300/api/org/all');
        const eventsWithTagsArray = response.data.map((org: any) => ({
          ...org,
          tags: typeof org.tags === 'string' ? org.tags.split(',') : org.tags,
        }));
        setOrg(eventsWithTagsArray);
      } catch (error) {
  ...
  const uniqueTags = Array.from(new Set(org.flatMap(org => org.tags)));
  const uniqueLocations = Array.from(new Set(org.map(org => org.location)));
  const filteredOrg = org.filter(org => {
    const matchesSearchTerm = org.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    org.tags.some(tag => (tagDescriptions[tag] || tag).toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesTag = selectedTag === "" || org.tags.includes(selectedTag);
    const matchesLocation = selectedLocation === "" || org.location === selectedLocation;
  
    // Ensure the time comparison uses the same format
    const orgDate = new Date(org.time).toISOString().split('T')[0]; // Convert event.time to YYYY-MM-DD
    const matchesTime = selectedTime === "" || orgDate === selectedTime;
...
export default OrganisationAll;