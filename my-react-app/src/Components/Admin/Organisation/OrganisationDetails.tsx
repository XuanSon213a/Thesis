...
const OrganisationDetails = () => {
  ...
  const [relatedOrg, setRelatedOrg] = useState<Org[]>([]);
  const [visibleOrgCount, setVisibleOrgCount] = useState(5);
 const [yesVotes, setYesVotes] = useState(0);
  const [noVotes, setNoVotes] = useState(0);
  const [hasVoted, setHasVoted] = useState(false);
  const [voteSuccess, setVoteSuccess] = useState(false);
  useEffect(() => {
    axios.get(`http://localhost:3300/api/org/${id}`)
      .then(response => {
        setOrg(response.data);
      })
      .catch(error => {
        console.error('There was an error fetching the organisation details!', error);
      });

    axios.get(`http://localhost:3300/api/org/${id}/comments`)
      .then(response => {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://localhost:3300/api/orgvotes/${id}`)
        .then(response => {
  ...
  useEffect(() => {
    if (org) {
      const interval = setInterval(() => {
        const newDate = new Date(org.time).getTime();
  ...
  useEffect(() => {
    if (org) {
      axios
        .get(`http://localhost:3300/api/org/related`, {
          params: { category: org.categories, tags: org.tags },
        })
        .then((response) => {
          // Ensure the response is an array
          setRelatedOrg(Array.isArray(response.data) ? response.data : []);
        })
        .catch((error) => {
          console.error('Error fetching related org:', error);
          setRelatedOrg([]); // Fallback to an empty array on error
        });
    }
  }, [org]);
  const handleShowMore = () => {
    setVisibleOrgCount((prevCount) => prevCount + 5); // Increase the visible news count by 5
  };
  ...
  const handleCommentSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    axios.post(`http://localhost:3300/api/org/${id}/comments`, { comment: newComment })
      .then(response => {
  ...
  const handleVote = (vote: 'yes' | 'no') => {
    ...
    axios.post('http://localhost:3300/api/orgvotes', { orgId: id, vote })
      .then(() => {
        setHasVoted(true);
        setVoteSuccess(true);
      ...
      .catch(error => {
        if (error.response && error.response.status === 400) {
          alert('You have already voted for this org.');
        } else {
      ...
      <div className="flex-1 md:mr-8">
        ...
        <div className="flex">
          ...
          <div className="ml-8">
            ...
            <p className="text-lg font-mono font-bold"><FontAwesomeIcon icon={faIndustry} /> {org.industry}</p>
        ...
        <div className="prose mt-8">
          <div dangerouslySetInnerHTML={{ __html: org.descr }} />
        </div>
        ...
        <div className="mt-8 p-4 border rounded-lg shadow-md bg-gray-100 max-w-sm">
          <h2 className="text-xl font-bold mb-2">Contact Info</h2>
          <hr className="my-2 border-gray-300" />
          <p className="text-lg font-mono">{org.contacter}</p>
          <p className="text-lg font-mono">{org.position}</p>
        </div>
        <div className="mt-4">
          <b>Tags</b>:
          <div className="flex flex-wrap gap-2 mt-2">
            {org.tags.split(',').map((tag, index) => (
              <span
                key={index}
  ...
  <h2 className="text-xl font-bold mb-4 text-center">Related Organisation</h2>
  <div className="space-y-4">
    {relatedOrg.map((related) => (
      <Link
        key={related.id}
        to={`/org/${related.id}`}
  ...
  {relatedOrg.length > visibleOrgCount && (
    <button
