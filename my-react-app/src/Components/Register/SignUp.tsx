...
export default function SignUp() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const uploadedFile = await uploadFile(file); // Gọi API upload ảnh
      setUploadPhoto(file);
      setFormData((prev) => ({ ...prev, profile_pic: uploadedFile.url }));
    }
  };
  React.useEffect(() => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            alert(`Error: ${errorResult.message || 'Registration failed'}`);
  ...
  //   event.preventDefault();
  //   axios.post('http://localhost/3300/register',values)
            ...
            <Card variant="outlined">
              ...
              >
                ...
                <FormControl>
                  ...
                  />
                  {uploadPhoto && (
                    <Typography>{uploadPhoto.name}</Typography>
                  )}
                </FormControl>
                <FormControlLabel
